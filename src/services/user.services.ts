...
module.exports = (app:Application, connection:Connection)=>{
  ...
  app.post("/users/login",async(req,res)=>{
      ...
      const user : Muser | undefined = await connection.getRepository(Muser).findOne({where:{username}})
      if(user === undefined){
...
app.post("/users/delete", async(req,res)=>{
    ...
    const _user: Muser | undefined = await connection.getRepository(Muser).findOne({where:{username:user.username}})
    if(_user !== undefined){
...
app.post("/users/disapprove", async(req,res)=>{
    ...
    const _user: User | undefined = await connection.getRepository(User).findOne({where:{username:user.username}})
    if(_user !== undefined){
